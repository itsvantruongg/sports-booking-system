...
const navItems = [
  ...
  { href: "/admin/revenue", label: "Doanh thu", icon: "account_balance" },
    ...
    <div className="flex h-screen overflow-hidden bg-[#fbf8ff]">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-[#f3f2ff] border-b border-[#e7e7f5] flex items-center justify-between px-6 z-40">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#434656] leading-none">Cổng Quản trị</p>
          </div>
        </div>
      ...
      <nav className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-[#f3f2ff] flex flex-col p-6 z-[100] transform transition-transform duration-300 ease-out shadow-2xl ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"
        ...
        <Link
          ...
          onClick={() => setMobileMenuOpen(false)}
          className="w-full bg-[#003ec7] text-white font-bold py-3 px-4 rounded-full hover:bg-[#0052ff] transition-colors shadow-md text-sm mb-6 flex items-center justify-center gap-2"
